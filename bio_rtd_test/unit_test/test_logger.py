...
class TestLogger(unittest.TestCase):
    ...
    def assert_print(self, msg: str, match=True):
        if match:
            ...
            self.assertNotEqual(msg, self.print_output.getvalue().split('\n')[-2])
    ...
    def assert_empty_function(self, fun):
        self.assertEqual(fun.__code__.co_code, self.empty_func.__code__.co_code)
    ...
    def test_default_logger(self):
        ...
        branch_list += [1, 2, 3]
        ref_dict = {"ab1": 11, "ab2": 22, "BranchTree": {"cab1": 122}, "BranchList": [1, 2, 3]}
        self.assertTrue(ref_dict == log.get_data_tree("MainTree2"))
    ...
    def test_data_storing_logger(self):
        ...
        log.d_data(data_tree, "ab1", 11)
        self.assert_print("Value set: ab1: 11", match=False)
        log.i_data(data_tree, "ab2", 22)
        self.assert_print("Value set: ab2: 22")
        ref_dict = {"ab1": 11, "ab2": 22}
        self.assertEqual(ref_dict, log.get_data_tree("MainTree3"))
        self.assertEqual(ref_dict, log.get_data_tree("MainTree3"))
        log.i_data(data_tree, "ab3", 33)
        self.assert_print("Value set: ab3: 33")
        ...
        self.assert_print("DummyUnitOp: Value set: ac1: 12", match=False)
        uo.data_write_to_data_storing_logger(data_tree, log.INFO, "ac2", 23)
        self.assert_print("DummyUnitOp: Value set: ac2: 23")
        ref_dict = {"ac1": 12, "ac2": 23}
        self.assertTrue(ref_dict == data_tree)
        self.assertTrue(ref_dict == uo.data_storing_logger.get_data_tree("MainTree4"))
        uo.data_write_to_data_storing_logger(data_tree, log.INFO, "ac3", 34)
        self.assert_print("DummyUnitOp: Value set: ac3: 34")
    ...
    def test_strict_logger(self):
        ...
        with self.assertRaises(RuntimeError, msg="w triggered RuntimeError via logger"):
            log.w("w triggered RuntimeError via logger")
        with self.assertRaises(RuntimeError, msg="e triggered RuntimeError via logger"):
            log.e("e triggered RuntimeError via logger")

        # Uo prefix
        uo = DummyUO()
        with self.assertRaises(RuntimeError, msg="w triggered RuntimeError"):
            uo.write_to_strict_logger(log.WARNING, "DummyUnitOp: w triggered RuntimeError")
