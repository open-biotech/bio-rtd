...
from bio_rtd import pdf, peak_shapes
...
class GaussianTest(unittest.TestCase):
    ...
    # define inlet with single specie
    def run_gauss_fixed_relative_with(self, t, rt_mean_list, sigma_list, cutoff) -> None:
        for rt_mean in rt_mean_list:
            for sigma in sigma_list:
                p = self.get_gauss_pdf_with_cutoff(t, rt_mean, sigma, cutoff)
...
class EmgTest(unittest.TestCase):
    ...
    def run_emg_fixed_dispersion(self, t, rt_mean_list, sigma_list, skew_list, cutoff) -> None:
    ...
    # define inlet with single specie
    # noinspection DuplicatedCode
    def run_emg_fixed_relative_width(self, t, rt_mean_list, sigma_list, skew_list, cutoff) -> None:
        for skew in skew_list:
            for sigma in sigma_list:
                for rt_mean in rt_mean_list:
                    if rt_mean < 1 / skew:  # peak max would be at t < 0
                        continue
                    p = self.get_emg_pdf_with_cutoff(t, rt_mean, sigma, skew, cutoff)
                    pd = pdf.ExpModGaussianFixedRelativeWidth(t,
                    ...
                    np.testing.assert_array_almost_equal(pd.get_p(), p, 3)
                    pd.update_pdf(rt_mean=rt_mean / 2)
                    p = self.get_emg_pdf_with_cutoff(t, rt_mean / 2, sigma / 2, skew, cutoff)
                    np.testing.assert_array_almost_equal(pd.get_p(), p, 3)
                    pd.update_pdf(rt_mean=rt_mean / 2, skew=skew / 2)
                    p = self.get_emg_pdf_with_cutoff(t, rt_mean / 2, sigma / 2, skew / 2, cutoff)
                    np.testing.assert_array_almost_equal(pd.get_p(), p, 3)
...
class TanksInSeriesTest(unittest.TestCase):
    ...
    # define inlet with single specie
    # noinspection DuplicatedCode
    def run_tanks_in_series(self, t, rt_mean_list, n_tank_list, cutoff) -> None:
